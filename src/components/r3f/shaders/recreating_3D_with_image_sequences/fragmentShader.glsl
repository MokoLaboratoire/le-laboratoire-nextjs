...
void main() {
	...
	vec2 subUv = getSubUv(vec2(4.0), vUv, index);
	vec2 subUvNext = getSubUv(vec2(4.0), vUv, index + 1.0);
	...
	float trans = smoothstep(0.0, 1.0, position.y + (1.0 - 2.0 * progress) + sin(position.x * 4.0 + progress * 6.0 + time) * mix(0.5, 0.1, abs(0.5 - position.x)) * 0.5 * smoothstep(0.0, 0.2, progress));

	float pixellisation = 200.0;
	float mixForUvs = (1.0 - trans) * 50.0 * progress;
	/* vec2 subUv1 = floor(subUv * pixellisation) / pixellisation;
	vec2 subUvNext1 = floor(subUvNext * pixellisation + 1.0) / pixellisation; */
	vec2 subUv1 = mix(subUv, floor(subUv * pixellisation) / pixellisation, mixForUvs);
	vec2 subUvNext1 = mix(subUv, floor(subUvNext * pixellisation + 1.0) / pixellisation, mixForUvs);

	vec4 color = getMap(uDiffuse, blend, subUv1, subUvNext1, displacement, displacementNext);

	color.a = trans * def.a;

	vec3 rainbow = vec3(1.0, 0.15, 0.15);
	vec3 hsv = rgb2hsv(rainbow);
	hsv.x += 0.8 * (vUv.y + trans - progress);
	rainbow = hsv2rgb(hsv);

	color.rgb = mix(color.rgb, rainbow, 0.5 * smoothstep(0.5, 0.0, abs(0.5 - color.a)));
	...
	/* gl_FragColor = vec4(vec3(mixForUvs), 1.0); */
}